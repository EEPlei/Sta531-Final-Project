...
ptc<-proc.time()
test<-mixture(test.3[[1]][[2]],test.3[[1]][[3]],MM,test.3[[1]][[1]],weights=c(0.03,0.05,0.92))
#test<-mixture(xmat,BW,MM,dataframe,weights=c(0.1,0,0.9))

Outstring<-as.vector(unique[test])
Outstringjoined<-paste(Outstring10,sep=" ",collapse = " ")
proc.time()-ptc

# 2 seconds per run for 13,500 words/580 unique

truerate<-scale(c(sort(table(text),decreasing=TRUE)[1:20]))
rate<-scale(c(sort(table(Outstring),decreasing=TRUE)[1:20]))
